...
export class FrontendSDK {
  ...
  async readContractEVM(contractAddress: string, abi: any[], functionName: string, args: any[] = []): Promise<any> {
    if (!this.evmProvider) {
      throw new Error('EVM provider not available.');
    }
  ...
  async estimateGasEVM(to: string, value: string = '0', data: string = '0x'): Promise<string> {
    if (!this.evmProvider) {
      throw new Error('EVM provider not available.');
    }
